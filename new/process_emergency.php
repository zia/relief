<?php
  ...
  function get_union_name($union_id) {
      global $pdo;
      $result = $pdo->prepare("SELECT union_name FROM unions WHERE id = ?");
  ...
  // Check if exit
  $results = $pdo->prepare("SELECT * FROM emergency WHERE nid = ? or mobile = ?");
  ...
  $sql = "INSERT INTO emergency(name, nid, mobile, unionp, ward, relief_type) VALUES (:name, :nid, :mobile, :unionp, :ward, :relief_type)";
